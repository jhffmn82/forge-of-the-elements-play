...
var OBJ_FX_RULES = [
  ...
  /* 2026-09-19, the Crypt and planes rebuild: painted candles and soul-green flames flicker where they burn, runes and
     plaques breathe, crystals glint (js/cryptpack.js reports the rect each piece is drawn in) */
  [/^candles-/,                                ['flicker']],
  [/^tomb-rune$/,                              ['glow', 'flames']],
  [/^wall-plaque$/,                            ['glow']],
  [/^(tomb-|dais-sarcophagus|grave-monument|grave-post|grave-pillar|sarcophagus|coffin|stairs-wide|urn-groups-)/, ['flames']],
  [/^rune-stone-/,                             ['glow']],
  [/^crystal-/,                                ['twinkle']],
  [/^stalagmite-(light|shadow|earth)$/,        ['twinkle']],
  [/^glow-mushrooms$/,                         ['glow']]
];
...
function objFxInfo(o){
  ...
  var W=Math.max(1,o.sw|0), H=Math.max(1,o.sh|0), info={bright:[], warm:null, green:null, tips:[], glow:null, flames:[]};
  try{
    var c=document.createElement('canvas'); c.width=W; c.height=H;
    var g=c.getContext('2d'); g.drawImage(o.img, o.sx, o.sy, o.sw, o.sh, 0, 0, W, H);
    var D=g.getImageData(0,0,W,H).data, cand=[], wx=0, wy=0, wn=0, wtop=H, gx=0, gy=0, gn=0, sr=0, sg=0, sb=0, sw=0, lowest={}, fl=[];
    for(var y=0;y<H;y++) for(var x=0;x<W;x++){
      ...
      /* the hot core of a painted flame: candle amber-to-white, or soul-fire green */
      if(r>235 && gg>170 && b<150 && r>b+100) fl.push([x,y,'w',r+gg]);
      else if(gg>190 && gg>r+70 && gg>b+50) fl.push([x,y,'g',gg*2]);
    }
    fl.sort(function(A,B){ return B[3]-A[3]; });
    for(var fi=0; fi<fl.length && info.flames.length<10; fi++){   /* one point per flame, the brightest pixel of each */
      var f0=fl[fi]; if(info.flames.every(function(e){ return Math.abs(e[0]*W-f0[0])+Math.abs(e[1]*H-f0[1])>=7; })) info.flames.push([f0[0]/W, f0[1]/H, f0[2]]);
    }
...
function objFxDraw(o, dx, dy, w, h, alpha, flip){
  ...
  fx.forEach(function(k){
    ...
    if(k==='flames'){                                          /* a small flickering halo on each painted flame */
      var u2=TS/64;
      ctx.globalCompositeOperation='lighter';
      I.flames.forEach(function(f, i){
        var fk=0.6+0.25*Math.sin(t*(8+i%3)+seed+i*1.7)+0.15*Math.sin(t*21+seed*2+i), fr=TS*0.2, cx=X(f[0]), cy=Y(f[1]);
        var col=f[2]==='g' ? '120,255,150' : '255,190,90';
        var fg=ctx.createRadialGradient(cx, cy, 0, cx, cy, fr);
        fg.addColorStop(0, 'rgba('+col+','+(0.34*fk*alpha)+')'); fg.addColorStop(1, 'rgba('+col+',0)');
        ctx.fillStyle=fg; ctx.fillRect(cx-fr, cy-fr, fr*2, fr*2);
        if(fk>0.72){ ctx.globalAlpha=(fk-0.72)*2.2*alpha; ctx.fillStyle=f[2]==='g' ? '#E0FFD0' : '#FFF4C8'; ctx.fillRect(Math.round(cx-u2), Math.round(cy-u2*2), Math.max(1,Math.round(u2*2)), Math.max(1,Math.round(u2*2))); ctx.globalAlpha=1; }
      });
