...
populateSpecialMonsters = function(){
  if(!deepMobsOn() || !floorMeta.boss) return _populateDeep();
  ...
  try{ _populateDeep(); } finally { halls.forEach(function(r){ r.role='boss'; }); }
...
kill = function(e, by){
  if(e && e.kind==='matron' && ents.indexOf(e)>=0){
    ...
    try{ r=_killDeep.apply(this, arguments); } finally { log=logSaved; }
  ...
  return _killDeep.apply(this, arguments);
};
...
floorIntro = function(){
  if(!deepMobsOn() || !floorMeta.boss) return _floorIntroDeep();
