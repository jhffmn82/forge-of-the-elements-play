...
drawGrassTile = function(x, y, px, py, alpha, layer, now){
  ...
  var t=ANIM.reduce ? 0 : (now||performance.now())/1000, pulse=0.65+0.35*Math.sin(t*1.8 + hash2(x,y,5)*6.28);
  ...
  ctx.restore();
  blitRaster(cachedRaster('shr@', x, y, function(a,b){ return shroomRaster(a,b,false); }), px, py, alpha);
};
var _drawGroundDecalShroom = drawGroundDecal;
drawGroundDecal = function(gv, x, y, px, py, alpha, now){
  if(gv===G_SHORT && cryptShrooms()){ blitRaster(cachedRaster('shs@', x, y, function(a,b){ return shroomRaster(a,b,true); }), px, py, alpha*0.9); return true; }
