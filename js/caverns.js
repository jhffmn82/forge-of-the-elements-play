...
var CAVE_PROP = {
  ...
  'bed-straw':['kobold-bedroll-1','kobold-bedroll-2'], 'rubble':['cl-rubble-1','cl-rubble-2','cl-rubble-3','cl-rubble-4'],
...
function caveDecorate(){
  ...
  var DEC=[['dc-lichen',3],['dc-spore-drift',2],['dc-mineral-vein',3],['dc-lichen',3],['dc-puddle',3]];
  for(y=1;y<MH-1;y++) for(x=1;x<MW-1;x++){
...
function caveKoboldCamp(r){
  ...
  for(var i=0;i<cells.length;i++){
    ...
    cavePlace(around[0].x, around[0].y, 'kobold-bedroll-1');
    if(rng()<0.6) cavePlace(around[1].x, around[1].y, 'kobold-bedroll-2');
