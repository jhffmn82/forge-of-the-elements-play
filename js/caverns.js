...
function caveRift(room, start){
  ...
  var cands=rows.filter(function(r){
    for(var k=0;k<w;k++){ var p=C(r.a,r.b+k); if(at(p.x,p.y)!==CHASM) return false;
      /* 2026-09-19: and the rift carries on past it on both sides - never a bridge laid at the tip of a rift */
      var pa=C(r.a-1,r.b+k), pb=C(r.a+1,r.b+k); if(at(pa.x,pa.y)!==CHASM || at(pb.x,pb.y)!==CHASM) return false; }
...
/* 2026-09-19: each floor has its own look - a few kinds of tall piece and of cluster, drawn from the full set -
   so things turn up uncommonly instead of everything on every map */
function caveTheme(){
  if(floorMeta.caveTheme) return floorMeta.caveTheme;
  function pickSome(list, n){ var l=list.slice(), out=[]; while(out.length<n && l.length){ var tot=l.reduce(function(a,t){ return a+t[t.length-1]; },0), r=rng()*tot;
    for(var i=0;i<l.length;i++){ r-=l[i][l[i].length-1]; if(r<=0){ out.push(l[i]); l.splice(i,1); break; } } } return out; }
  var th={tall:pickSome(CAVE_TALL, ri(2,3)), clusters:pickSome([['small-mushrooms',3],['crystal-shards',2],['rubble',3],['cave-pearls',1.2]], 2).map(function(c){ return c[0]; }), miner:rng()<0.3};
  return (floorMeta.caveTheme=th);
}
function caveTallPiece(room){
  var TALL=caveTheme().tall;
  var tot=TALL.reduce(function(a,t){ return a+t[2]; },0), r=rng()*tot, t=TALL[0];
  for(var i=0;i<TALL.length;i++){ r-=TALL[i][2]; if(r<=0){ t=TALL[i]; break; } }
...
function caveDecorate(){
  ...
  var DEC=[['dc-mineral-vein',3],['dc-puddle',3]];   /* lichen grows in patches, below */
  for(y=1;y<MH-1;y++) for(x=1;x<MW-1;x++){
    if(at(x,y)!==FLOOR) continue;
    var n=caveNoise(x*0.28, y*0.28, salt);
    if(n<0.58 || hash2(x,y,salt+3)>0.1) continue;   /* 2026-09-19: half as many decals */
    var kind=DEC[Math.floor(caveNoise(x*0.12+9, y*0.12, salt+7)*DEC.length*0.999)];
    var nm=kind[0]+'-'+(1+Math.floor(hash2(x,y,salt+5)*kind[1]));
    if(nm==='dc-puddle-2') nm='dc-puddle-'+(hash2(x,y,salt+6)<0.5 ? 1 : 3);   /* 2026-09-19: the crescent puddle read as a scythe */
    deco.decals.push({x:x, y:y, n:nm});
  }
  /* 2026-09-19: lichen only as a patch of 3-4 joined tiles (faint, see cavernrender.js), in about a third of the
     chambers - or not at all */
  rooms.forEach(function(room){
    if(!room.cave || rng()>0.35) return;
    var cells=shuffled(caveRoomCells(room).filter(function(c){ return at(c.x,c.y)===FLOOR; }));
    var start=cells[0]; if(!start) return;
    var patch=[start], want=ri(3,4), tries=0, key={}; key[start.x+','+start.y]=1;
    while(patch.length<want && tries++<30){
      var from=patch[Math.floor(rng()*patch.length)], d=[[1,0],[-1,0],[0,1],[0,-1]][Math.floor(rng()*4)], nx=from.x+d[0], ny=from.y+d[1];
      if(key[nx+','+ny] || at(nx,ny)!==FLOOR || floorMeta.caveRoom[idxOf(nx,ny)]!==room.id) continue;
      key[nx+','+ny]=1; patch.push({x:nx, y:ny});
    }
    if(patch.length<3) return;
    patch.forEach(function(c){ deco.decals.push({x:c.x, y:c.y, n:'dc-lichen-'+(1+Math.floor(hash2(c.x,c.y,salt+9)*3))}); });
  });
  ...
  var TH=caveTheme(), CL=[['small-mushrooms',3],['crystal-shards',2],['rubble',3],['cave-pearls',0.6],['lost-miner',0.5]].filter(function(c){ return TH.clusters.indexOf(c[0])>=0 || (c[0]==='lost-miner' && TH.miner); }), miners=0;   /* this floor's kinds only; glow moss grows in streaks, below */
  rooms.forEach(function(room){
    if(!room.cave) return;
    var groups = room.arena ? ri(1,2) : Math.min(2, Math.floor(room.n/40) + (rng()<0.4?1:0));
...
decoratePlain = function(r){
  ...
  if(rng()<0.4) caveTallPiece(r);
  if(r.n>=50 && rng()<0.2) caveTallPiece(r);
  if(!floorMeta.koboldCamp && r.role!=='start' && r.n>=30 && rng()<0.15) caveKoboldCamp(r);
